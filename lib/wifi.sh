...
wifi_show_menu() {
    local selected=$1
    local items=("Disable Network" "Enable Network" "Show Network Info" "Back to Main Menu")
...
network_off() {
    ...
    read -p "Enter interface name (e.g., ens33): " interface
    ...
    echo ""
    read -p "Disable $interface? (y/n): " confirm
    ...
    if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
        ...
        if [ $? -eq 0 ]; then
            display_success "Network disabled"
            log_message "INFO" "Network disabled on $interface"
        else
            display_error "Failed to disable network"
...
network_on() {
    ...
    if [ $? -eq 0 ]; then
        display_success "Network enabled"
        
        echo ""
        display_info "Connection details:"
        echo ""
        nmcli device show "$interface" | grep "IP4"
        
        log_message "INFO" "Network enabled on $interface"
    else
        display_error "Failed to enable network"
...
network_info() {
    ...
    display_info "Network Interfaces:"
    echo ""
    nmcli device status
